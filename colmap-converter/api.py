...
@app.post("/generate_map_depths")
async def run_depth_maps_generation(request: ConvertRequest):
    command = f"python3 /workspace/Depth-Anything-V2/run.py --encoder vitl --pred-only --grayscale --img-path {request.input_dir}/images --outdir <output path> -s {request.input_dir}/depth_maps"
    logger.info(f"Starting map depths generation for directory: {request.input_dir}")
    ...
    if process.returncode != 0:
        error_message = "Map depths generation failed"
        logger.error(error_message)
        raise HTTPException(
            status_code=500,
            detail={"error": error_message, "stderr": "Process error"}
    ...
    logger.info("Map depths generation completed successfully")
    return {"message": "Conversion completed successfully"}
